...
export async function sendInvitationEmail(email: string, inviteLink: string, inviterName: string) {
  const mailOptions = {
    ...
    to: email,
    subject: `You've been invited to join BAP Workspace`,
    html: `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <title>You're Invited to BAP Workspace</title>
      </head>
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #ffffff; padding: 40px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
          ...
          <p style="color: #666;"><strong>${inviterName}</strong> has invited you to join their team on BAP Workspace.</p>
          <p style="color: #666;">Click the button below to accept the invitation and set up your account:</p>
          <div style="text-align: center; margin: 30px 0;">
            <a href="${inviteLink}" style="display: inline-block; background: linear-gradient(135deg, #BA6675 0%, #95739F 100%); color: white; padding: 14px 30px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px;">Accept Invitation</a>
          </div>
          <p style="color: #999; font-size: 12px; margin-top: 30px; border-top: 1px solid #eee; padding-top: 20px;">This invitation expires in 7 days. If you didn't expect this invitation, you can safely ignore this email.</p>
        </div>
      </body>
      </html>
    `,
